    ...
    public static String encrypt(String text, String key) {
        ...
        for (char c : text.toCharArray()) {
            ...
            if (RUSSIAN_ALPHABET.indexOf(originalChar) >= 0) {
                int shift = RUSSIAN_ALPHABET.indexOf(Character.toUpperCase(key.charAt(keyIndex % key.length())));
                ...
                int shift = LATIN_ALPHABET.indexOf(Character.toUpperCase(key.charAt(keyIndex % key.length())));
    ...
    public static String decrypt(String text, String key) {
        ...
        for (char c : text.toCharArray()) {
            ...
            if (RUSSIAN_ALPHABET.indexOf(originalChar) >= 0) {
                int shift = RUSSIAN_ALPHABET.indexOf(Character.toUpperCase(key.charAt(keyIndex % key.length())));
                int originalIndex = RUSSIAN_ALPHABET.indexOf(originalChar);
                int newIndex = (originalIndex - shift + RUSSIAN_ALPHABET.length()) % RUSSIAN_ALPHABET.length();
